...
export function CareerTimeline({
  ...
  return (
    <section className={bare ? '' : 'mt-20'}>
      {!bare && (
        <Reveal>
          <p className="font-mono text-[11px] uppercase tracking-kicker text-accent">From day one</p>
          ...
          {career.map((c, i) => (
            <Reveal key={c.id ?? i} delay={(i % 6) * 0.05}>
              <div className="relative">
                <span className="absolute -left-9 top-[9px] h-2.5 w-2.5 rounded-full bg-accent ring-4 ring-ink sm:-left-12" />
